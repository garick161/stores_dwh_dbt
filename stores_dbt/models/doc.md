...
### tests:
- #### check_date 
Тест проверят валидность даты. Плохой кейс - дата больше текущей.  Если тест провален, построение зависимых моделей pipeline будет преостановлено,  чтобы избежать получения некорректных показателей в слое витрин.


- #### check_daily_revenue
Тест проверяет допустимое значение дневной выручки для конкретного магазина на основе накопленной статистики. Нормальный дипазон значений: среднее значение для выручки +/- 2 * стандартных отклонения, что покрывает 95% дипазон валидных значений. Все что выходит за пределы этого диапазона можно считать выбросами и заслуживает отдельного рассмотрения. Загрузка данных и построения витрин не будет прервано, но будет предупреждение о том что тест не пройден.
...
Тест показывает магазины с нулевым трафиком за день. Загрузку данных и построение отчетов не останавливает. Выдает предупреждение, что тест не пройден. И нужно отдельно разбираться бизнесу почему с конретного магазина сегодня нет данных.
...
{% docs plant_revenue_avg %}
### Накопительная статистика по магазинам за день
> Обновляется каждый раз при выполнении модели за указанный период

|plant|avg_revenue|std_revenue|
|-----|----------:|----------:|
|M004 |   12810.71|   13754.67|
|M006 |    9383.00|    9629.05|
|M008 |    8423.14|    5019.22|
|M001 |   11220.71|   11154.51|
|M007 |    7202.79|    8272.24|
