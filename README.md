## Введение
В данном проекте показана реализация итогового задания после прохождения курса по `GreenPlum` с использованием `DBT` в качестве инструмента для трансформации данных.  
Первая часть будет вводная, с описанием некоторых интересных возможностей `DBT`. Общие подходы к разработке, реализации CI/CD и варианты взаимодействия с аналитиками данных при проектировании витрин.  
...
Современные LLM модели (ChatGPT и другие) могут достаточно хорошо писать SQL запросы. Можно составить промт для такой модели, который будет содержать только структуру детального слоя (только DDL таблиц), описание таблиц и их полей. Температуру такой модели выставить на минимум (нам не нужна креативность, а больше важна точность и стабильность). Интерфейс взаимодействия с такой моделью может быть любым.
...
В данном проекте я использую 3 теста:
...
## Выводы
- `BDT` универсальный инструмент, который заслуживает внимания. Универсальный потому, что формирует общий единообразный подход к трансформации данных в хранилище, не зависимо на какой базе данных оно построено (Postgres, GreenPlum, ClickHouse, S3 . . .)
